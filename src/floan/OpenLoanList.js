...
export default function OpenLoanList({ goToHomeView, goToRequesterInfoView, fundLoad, openLoans }) {

    const rows = openLoans.map(loan => {
        return (
            <TableRow key={loan.id}>
                <TableCell>{loan.id}</TableCell>
                <TableCell><a onClick={() => goToRequesterInfoView(loan.request.requester.id)}>{loan.request.requester.id}</a></TableCell>
